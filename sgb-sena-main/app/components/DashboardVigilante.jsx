...
// Initial state is empty, will be populated by API
const initialMockData = [];

export default function DashboardVigilante({ user }) {
    ...
    const [authorizations, setAuthorizations] = useState(initialMockData);
    ...
    const fetchAuthorizations = async () => {
        setIsSearching(true);
        try {
            const response = await fetch('/api/vigilante/autorizaciones');
            const data = await response.json();
            if (data.success) {
                setAuthorizations(data.autorizaciones);
            }
        } catch (error) {
    ...
    // Search logic (client-side for now, can be moved to API)
    const handleSearch = async () => {
        if (!searchQuery.trim()) {
            // If empty search, reset current view but keep data
            setCurrentAuth(null);
            return;
        }
        ...
        const found = authorizations.find(
            auth =>
                ...
                auth.documento.includes(searchQuery)
        );
    ...
    const handleRegisterExit = async () => {
        ...
        try {
            ...
            const vigilanteDoc = userData.documento || '0'; // Fallback if not found

            const response = await fetch('/api/vigilante/registros', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    tipo: 'SALIDA',
                    autorizacionId: currentAuth.original_id,
                    documentoVigilante: vigilanteDoc,
                    observaciones: observations,
                    bienes: currentAuth.bienes.filter(b => selectedItems.includes(b.serial))
                })
            });
    ...
    const handleRegisterReentry = async () => {
        if (!currentAuth) return;

        try {
            const userData = JSON.parse(localStorage.getItem('user') || '{}');
            const vigilanteDoc = userData.documento || '0';
            ...
            if (result.success) {
                ...
                alert('Error al registrar reingreso: ' + result.error);
            }
        } catch (error) {
            console.error('Error registering reentry:', error);
            alert('Error de conexión al registrar reingreso');
        }
    };
    ...
    const handleSelectFromTable = (authId) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-[#F9FAFB] overflow-auto">
            {/* Header */}
                        ...
                        >
                            <div className="flex flex-col items-end mr-2">
                                <span className="text-white font-bold text-sm leading-tight">
                                    {user ? `${user.nombres} ${user.apellidos}` : 'Usuario Vigilante'}
                                </span>
                                <span className="text-white/80 text-xs font-medium">
                                    Vigilante
                                </span>
                            </div>
                            <User className="h-8 w-8 text-white bg-white/20 rounded-full p-1" />
