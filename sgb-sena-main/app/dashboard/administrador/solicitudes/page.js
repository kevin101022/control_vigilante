...
export default function SolicitudesAdministrador() {
  ...
  const handleFirmar = async (aprobar) => {
    ...
    if (!confirm(`¿Confirmar ${aprobar ? 'aprobación final' : 'rechazo'}?`)) return;
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        alert(aprobar ? 'Solicitud aprobada completamente' : 'Solicitud rechazada');
  ...
  const puedeFiremar = async (solicitud) => {
    // Verificar si el cuentadante Y el coordinador ya firmaron
    try {
      ...
      if (data.success) {
        ...
        const firmaCoordinador = data.firmas.find(f => f.rol_usuario === 'coordinador');
        return firmaCuentadante && firmaCuentadante.firma === true &&
               firmaCoordinador && firmaCoordinador.firma === true;
      }
    } catch (error) {
      console.error('Error:', error);
    }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Aprobación final de solicitudes</p>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button
                ...
                title={!puedeFiremarState ? 'El cuentadante y coordinador deben firmar primero' : ''}
              >
                Rechazar
              </button>
              <button
